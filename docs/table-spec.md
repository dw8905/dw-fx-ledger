...
기준: SQLAlchemy models + Alembic migrations `20260604_0001`, `20260605_0002`, `20260607_0003`, `20260607_0004`, `20260607_0005`, `20260607_0006`.
...
| `item_codes` | 아이템 코드 마스터 |
| `item_trades` | 아이템 매수/매도 손익 계산 기록 |
...
아이템을 코드 단위로 묶기 위한 전역 관리자 마스터 테이블입니다.

| 컬럼 | 타입 | Null | Key | Default | 설명 |
|---|---|---:|---|---|---|
| `item_code_id` | `bigint` | N | PK | autoincrement | 아이템 코드 ID |
| `user_id` | `bigint` | Y | FK `users.user_id`, IX |  | 과거 사용자별 코드 호환 컬럼. 전역 코드는 NULL |
| `item_code` | `varchar(80)` | N | Unique partial |  | 전역 아이템 코드. 관리자 등록 시 `ITEM-000001` 형식으로 자동 생성 |
| `item_name` | `varchar(120)` | N | IX |  | 물건명 |
| `memo` | `text` | Y |  |  | 메모 |
...
| 컬럼 | 타입 | Null | Key | Default | 설명 |
|---|---|---:|---|---|---|
| `item_trade_id` | `bigint` | N | PK | autoincrement | 아이템 거래 기록 ID |
| `user_id` | `bigint` | N | FK `users.user_id`, IX composite |  | 기록 소유 사용자 |
| `item_code_id` | `bigint` | Y | FK `item_codes.item_code_id`, IX |  | 아이템 코드 |
