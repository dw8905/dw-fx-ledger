...
매수 로트 등록부터 부분매도, 다중 로트 차감, 매도 취소, 이벤트 이력, 엑셀형 원장 조회까지 지원합니다. 추가로 게임/마켓 아이템 매수·매도·재고 조정 기록을 평균단가 방식으로 관리하는 아이템 거래 기능과 read-only 중심 Admin 사이트를 포함합니다.
...
- `apps/web`: 로그인, 게시판, FX 매수/매도/원장/Dev Lab, 아이템 거래 화면을 제공합니다.
- `apps/admin`: 관리자 로그인, 사용자/게시글/FX 원장/이벤트/아이템 코드 관리 화면을 제공합니다.
- `apps/api`: 인증, 게시판, FX, 아이템 거래, Admin API, SQLAlchemy 모델, Alembic migration, pytest를 포함합니다.
...
- 상단 메뉴에서는 `FX` 하나로 진입하고, FX 내부 탭에서 매수/매도/원장/Lab을 이동합니다.
- 매수 로트 등록
- 매수 로트 수정
- 매수 로트 삭제
...
- 특정 사용자 FX 원장 조회
- FX lot event 조회
- 아이템 코드 마스터 등록/수정/비활성화
- 아이템명 중복 등록 방지

아이템 거래

- `/item-trades`
- 관리자 아이템 코드 마스터를 웹에서 자동완성으로 선택합니다.
- 매수, 매도, 아이템별 재고관리 탭을 제공합니다.
...
- 같은 아이템명 중복 등록/수정은 API에서 차단합니다.

`item_trades`

- 아이템 코드별 매수/매도/재고조정 기록과 평균단가 방식 손익 계산 결과를 저장합니다.
- 최소 이득 판매가, 수수료율, 수수료, 수수료 차감 후 금액, 손익, 거래 후 보유 수량을 기록합니다.
- `trade_type`은 `buy`, `sell`, `adjustment`를 사용합니다.
- 매수/매도/조정 취소는 `cancelled` 상태로 남기고, 같은 아이템의 active 거래 기준으로 재고와 평균단가를 재계산합니다.
...
- `/admin/item-codes`: 아이템 코드 마스터 관리
...
- Admin은 현재 조회와 아이템 코드 마스터 관리 중심입니다.
- Admin에서 FX 장부 자체를 수정/삭제하는 기능은 없습니다.
- 기존 중복 아이템 코드 중 거래가 연결된 항목은 병합 기준을 정해 별도 정리해야 합니다.
